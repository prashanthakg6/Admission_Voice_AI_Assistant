...
class BrainController:
    ...
    def handle(self, text: str, session) -> str:
        ...
        session.merge_slots(extracted)

        # TODO 5: check missing slots
        missing = [
                s for s in required_slots
                if s not in session.slots or session.slots[s] in (None, "", [])
            ]
    ...
    # TODO: Have to implement more words these words are not enough for production level intent detection
    ...
    def next_state(self, current_state, intent):
        if current_state not in FSM_TRANSITIONS:
            return current_state
        return FSM_TRANSITIONS[current_state].get(intent, current_state)
    ...
    def load_knowledge_for_state(self, state):
        ...
        topic = mapping.get(state)
        if not topic:
            return {}

        return self.knowledge.load(topic)
